	...
	public VComprobacion(IngresoLaby igl) {
		...
		btnComp.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try{
					int id = Integer.parseInt(txtId.getText()); // Pasem el
																// contingut del
																// txtId a
																// Integer.
					String cognom = txtCog.getText(); // Ens guardem el text del
														// txtCog.
					cognom = cognom.toUpperCase(); // Ho pasem a majúscules.
					txtComp.setText(il.getCod(id, cognom)); // Utilitzem el
															// mètod que
															// especifica el
															// enunciat per a
															// acabar el examen.
				}catch(NullPointerException npe){//Controlem que estiga tot ben introduit.
					JOptionPane.showMessageDialog(null, "No has introduït totes les dades!");
				}catch(NumberFormatException nfe){//Controlem que la ID estiga ben introduida.
					JOptionPane.showMessageDialog(null, "No has introduït la ID!");
				}
